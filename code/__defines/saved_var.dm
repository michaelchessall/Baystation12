 ...
 */
#define SAVED_VAR(CLASSPATH, VARNAME) CLASSPATH/proc/zz_compilerVarExistsTest_##VARNAME(){.=src.##VARNAME;}; /singleton/saved_variables##CLASSPATH/make_saved_variables(list/saved){LAZYDISTINCTADD(saved, #VARNAME); return ..(saved);}
...
/singleton/saved_variables##CLASSPATH/make_saved_variables(list/saved){\
 ...
 */
#define SAVED_INSTANCIATE_ONLY(CLASSPATH) /singleton/saved_variables##CLASSPATH/make_saved_variables(list/saved){if(length(cached)){cached.Cut();}}

/**
 * Save only the incon_state for this object, and ignore any other variable set by ancestors.
 */
#define SAVED_ICON_STATE_ONLY(CLASSPATH) /singleton/saved_variables##CLASSPATH/make_saved_variables(list/saved){if(length(cached)){cached.Cut();}cached=list("icon_state", "icon", "opacity", "color");}

/**
 * Mark the type as being turned into a single json array when saved.
 */
#define SAVED_FLATTEN(CLASSPATH) /singleton/saved_variables##CLASSPATH/should_flatten = TRUE;

...
/datum/controller/subsystem/persistence/proc/_prepare_zlevels_indexing()
	...
	try
		...
		for(var/z in GLOB.using_map.player_levels) //#FIXME: That logic is flawed. We got levels that aren't dynamic and aren't station levels!!!!
		...
		for(var/z in saved_levels)
			...
			z_level.index = z
			if(z in GLOB.using_map.player_levels) //#FIXME: That logic is flawed. We got levels that aren't dynamic and aren't station levels!!!!
		...
		for(var/area/A in saved_areas)
			for(var/turf/T in A)
				...
				z_level.index = T.z
				z_level.dynamic = TRUE
				if("[T.z]" in GLOB.known_overmap_sectors)
					var/obj/effect/overmap = GLOB.known_overmap_sectors["[T.z]"]
...
/datum/controller/subsystem/persistence/proc/_save_turfs(var/list/z_transform, var/instanceid)
	...
	for(var/z in saved_levels)
		...
		if(!("[z]" in seen_zlevels))
			...
			z_level.default_turf = default_turf
			z_level.index = z
			if(!HasAbove(z) && HasBelow(z))
				var/height_calc = z
				var/height = 1
				while(HasBelow(height_calc))
					++height
					--height_calc
				z_level.height = height
		try
...
/datum/controller/subsystem/persistence/proc/_before_save(var/save_initiator)
	...
	save_log_id = null
	SSpersistence.saved_levels = GLOB.using_map.player_levels.Copy()

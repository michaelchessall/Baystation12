...
/serializer/sql/SerializeList(var/list/_list, var/datum/list_parent, var/instanceid = 0)
	...
	for(var/key in _list)
		...
		else if(istype(key, /singleton))
			var/singleton/key_d = key
...
#ifdef SAVE_DEBUG
		if(verbose_logging)
			to_world_log("(SerializeListElem-Done) ([l_i],\"[KV]\",'[KT]',\"[EV]\",\"[ET]\",[instanceid])")
#endif
		found_element = TRUE
...
/serializer/sql/DeserializeList(var/raw_list, var/debugger = 1)

...
/datum/nano_module/program/faction_core/proc/try_connect()
	...
	if(connected_faction)
		if(connected_faction.uid != logistic_processor.faction_uid || connected_faction.password != logistic_processor.faction_password)
			connected = 0
			connected_faction = null
			menu = 1
			return
		...
		connected_faction = get_faction(logistic_processor.faction_uid)
		if(connected_faction)
			if(connected_faction.password != logistic_processor.faction_password)
				connected = 0
				connected_faction = null
				menu = 1
				return
...
/datum/nano_module/program/faction_core/Topic(href, href_list)
	...
	switch (href_list["action"])
		if("connect")
			...
			var/curr_faction_password = logistic_processor.faction_password
			var/select_faction_uid = input(usr, "Enter Lognet UID")
			var/select_faction_password = input(usr, "Enter Lognet Password")
			if(curr_faction_password == logistic_processor.faction_password && curr_faction_uid == logistic_processor.faction_uid)
				logistic_processor.faction_uid = select_faction_uid
				logistic_processor.faction_password = select_faction_password
				if(!try_connect())
					wrong_connection = 1
			else
		...
		if("disconnect")
			...
			if(curr_attempted_password == attempted_password)
				if(select_attempted_password != connected_faction.password)
					...
					logistic_processor.faction_uid = ""
					logistic_processor.faction_password = ""
					attempted_password = ""
			else
				to_chat(usr, "Your inputs expired because someone used the terminal first.")
		if("log_out")

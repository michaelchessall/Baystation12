...
/datum/computer_file/program/business/proc/open_file(var/filename)
	var/datum/computer_file/data/F = computer.get_file(filename)
	if(F)
		return F

/datum/computer_file/program/business/proc/save_file(var/filename, var/filedata)
	. = computer.save_file(filename, filedata, /datum/computer_file/data/text)
			...
			if(transactions.len)
				for(var/i=0; i<10; i++)
					...
					if(minus < transactions.len)
						...
						if(T && istype(T))
							formatted_transactions[++formatted_transactions.len] = list("date" = T.date, "time" = T.time, "target_name" = T.target.account_name, "purpose" = T.purpose, "amount" = T.amount ? T.amount : 0)
...
/datum/nano_module/program/business/proc/open_file(var/filename)
	var/datum/computer_file/program/business/root_program = program
	var/datum/computer_file/data/F = root_program.open_file(filename)
	if(F)
...
/datum/nano_module/program/business/proc/save_file(var/filename)
	var/datum/computer_file/program/business/root_program = program
	. = root_program.save_file(filename, filedata)
	if(.)
		is_edited = 0

/datum/nano_module/program/business/proc/create_file(var/newname, var/data = "")
	create_file(newname, data, /datum/computer_file/data/text)

...
/datum/nano_module/program/id_management/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui = null, force_open = 1, state = GLOB.default_state)
	...
	if(program && program.computer)
		data["have_printer"] = !!program.computer.get_component(PART_PRINTER)
	else
		data["have_printer"] = 0
...
/datum/computer_file/program/id_management/Topic(href, href_list)
	...
	switch(href_list["action"])
		if("print_card")
			...
			var/turf/T = get_turf(computer.get_component(PART_NETWORK))
			playsound(T, pick('sound/items/polaroid1.ogg', 'sound/items/polaroid2.ogg'), 75, 1, -3)
			id.forceMove(T)
		if("devalidate_card")

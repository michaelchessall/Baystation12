...
/turf/StandardWrite(f)
	...
	..()
	if((Uptime() - starttime)/10 > 2)
		to_world("[src.type] took [(Uptime() - starttime)/10] seconds to save at [x] [y] [z]")
...
//	var/starttime = Uptime()
	...
	//	debug_data["[src.type]"] = list(amount++,time+((Uptime() - starttime)/10))
	else
	//	debug_data["[src.type]"] = list(1,(Uptime() - starttime)/10)
...
/turf/StandardRead(var/savefile/f)
	...
	if("[src.type]" in debug_data)
		var/amount = debug_data["[src.type]"][1]
		var/time = debug_data["[src.type]"][2]
		debug_data["[src.type]"] = list(amount+1,time+((Uptime() - starttime)/10))
	else
		debug_data["[src.type]"] = list(1,(Uptime() - starttime)/10)
	if((Uptime() - starttime)/10 > 29)
		to_world("[src.type] took [(Uptime() - starttime)/10] seconds to load at [x] [y] [z]")
...
/proc/Save_Overmap()
	...
	to_world("Saving Completed in [(Uptime() - starttime)/10] seconds!")
...
/proc/Load_Overmap()
	...
	var/starttime2 = Uptime()
	var/breakout = 0

	while(!f.eof && !breakout)
		sleep(-1)
		if(((Uptime() - starttime2)/10) > 300)
			breakout = 1
		f >> ve
	message_admins("Loading Overmap Completed in [(Uptime() - starttime2)/10] seconds!")
	f = null
	...
	for(var/z in zlevels_to_load)
		...
		while(!f.eof && !breakout)
			...
			if(((Uptime() - starttime2)/10) > 300)
				breakout = 1
			f >> ve
		message_admins("Loading Zlevel [z] Completed in [(Uptime() - starttime2)/10] seconds!")
		f = null
	...
	to_world("Loading Completed in [(Uptime() - starttime)/10] seconds!")

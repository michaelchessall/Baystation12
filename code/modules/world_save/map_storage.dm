...
var/world_currently_loading = 0
/datum/admins/proc/loadnow()
	set category = "Server"
	set desc="Loads the Station"
	set name="Load Station"
	world_currently_loading = 1
	if(!check_rights(R_ADMIN))
		return
	var/list/ss_to_wake = list()
...
var/list/zlevels_to_save
var/list/zlevels_to_load
/datum/shuttle/var/list/formatted_areas = list()
/datum/shuttle/var/curr_x
/datum/shuttle/var/curr_y
/datum/shuttle/var/curr_z
/datum/shuttle/var/trans_x
/datum/shuttle/var/trans_y
/datum/shuttle/var/trans_z


/proc/Save_Overmap()
	zlevels_to_save = list()
	...
	while(!backup)
		if(fexists("backups/[dir]/z1.sav"))
			...
			backup = 1
	found_vars = list()
	fcopy("map_saves/overmap.sav", "backups/[dir]/overmap.sav")
	fdel("map_saves/overmap.sav")
	var/savefile/f = new("map_saves/overmap.sav")
	for(var/x in 1 to world.maxx step 20)
		for(var/y in 1 to world.maxy step 20)
			Save_Chunk(x,y, GLOB.using_map.overmap_z, f)
	f = null
	sleep(1 SECOND)
	message_admins("^# of ZLEVELS TO SAVE: [zlevels_to_save.len]")
	for(var/z in zlevels_to_save)
		message_admins("Saving Z [z]")
		fcopy("map_saves/z[z].sav", "backups/[dir]/z[z].sav")
		fdel("map_saves/z[z].sav")
		var/savefile/fa = new("map_saves/z[z].sav")
		for(var/x in 1 to world.maxx step 20)
			for(var/y in 1 to world.maxy step 20)
				Save_Chunk(x,y,z, fa)
		fa = null
	...
	var/savefile/fe = new("map_saves/extras.sav")
	...
	for(var/zone/Z in zones_to_save)
		...
		zones |= Z
	var/list/formatted_shuttles = list()
	message_admins("SHUTTLE SAVE ATTEMPT [SSshuttle.shuttles.len]")
	for(var/Su in SSshuttle.shuttles)
		var/datum/shuttle/S = SSshuttle.shuttles[Su]
		S.formatted_areas.Cut()
		
		if(S.current_location)
			S.curr_x = S.current_location.x
			S.curr_y = S.current_location.y
			S.curr_z = S.current_location.z
		if(istype(S, /datum/shuttle/autodock))
			var/datum/shuttle/autodock/S2 = S
			if(S2.landmark_transition)
				S.trans_x = S2.landmark_transition.x
				S.trans_y = S2.landmark_transition.y
				S.trans_z = S2.landmark_transition.z
		...
		for(var/area/A in S.shuttle_area)
			...
			holder.name = A.name
			holder.turfs = A.get_turf_coords()
			S.formatted_areas += holder
		formatted_shuttles |= S
	to_file(fe["shuttles"], formatted_shuttles)
	to_file(fe["factions"],GLOB.all_world_factions)
	to_file(fe["businesses"],GLOB.all_business)
	to_file(fe["zones"],zones)
	to_file(fe["areas"],formatted_areas)
	to_file(fe["zlevels"], zlevels_to_save)
	Save_Records(dir)

	to_file(fe["next_account_number"],next_account_number)
	fe = null
...
/proc/Load_Overmap()
	...
	if(!GLOB.all_crew_records)		GLOB.all_crew_records = list()
	...
	var/list/zlevel_list
	from_file(f["zlevels"],zlevel_list)
	if(zlevel_list)
		skip_overmap = 1
		for(var/z in zlevel_list)
			if(world.maxz < z)
				for(var/i in world.maxz to z)
					world.maxz++
	var/list/areas
	...
	for(var/datum/area_holder/holder in areas)
		...
		try
			...
			message_admins("AREA [A.name] caused an exception: [e] on [e.file]:[e.line] during load")
	f = null
	f = new("map_saves/overmap.sav")
	...
	while(!f.eof && !breakout)
		...
		if(((REALTIMEOFDAY - starttime2)/10) > 300)
			breakout = 1
		f >> ve
	message_admins("Loading Overmap Completed in [(REALTIMEOFDAY - starttime2)/10] seconds!")
	f = null
	message_admins("[zlevels_to_load] ZLEVELS TO LOAD")
	for(var/z in zlevels_to_load)
	...
	var/list/shuttles = f["shuttles"]
	for(var/datum/shuttle/S in shuttles)
		message_admins("Loaded shuttle [S.name]")
		log_world("Loaded shuttle [S.name]")
		if(islist(S.shuttle_area))
			S.shuttle_area.Cut()
		else
			S.shuttle_area = list()
		for(var/datum/area_holder/holder in S.formatted_areas)
			var/area/A = new holder.area_type
			A.name = holder.name
			try
				var/list/turfs = list()
				...
				A.contents.Add(turfs)
				S.shuttle_area |= A
				log_world("[A.name] loaded with [A.contents]")
			catch(var/exception/e)
				message_admins("AREA [A.name] caused an exception: [e] on [e.file]:[e.line] during load")
		if(S.current_location)
			S.current_location.x = S.curr_x
			S.current_location.y = S.curr_y
			S.current_location.z = S.curr_z
		if(istype(S, /datum/shuttle/autodock))
			var/datum/shuttle/autodock/S2 = S
			if(S2.landmark_transition && istype(S2.landmark_transition, /obj/))
				S2.landmark_transition.x = S2.trans_x
				S2.landmark_transition.y = S2.trans_y
				S2.landmark_transition.z = S2.trans_z
		SSshuttle.shuttles[S.name] = S
		SSshuttle.process_shuttles |= S
	var/list/zones
	SSskybox.rebuild_skyboxes(zlevels_to_load)
	...
	for(var/y in yi to yi + 20)
		for(var/x in xi to xi + 20)
			try
				...
				log_world("LOAD_CHUNK RUNTIME!! ([ex])")

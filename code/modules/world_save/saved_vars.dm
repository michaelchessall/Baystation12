...
var/global/SAVED_VARIABLES_TYPE = /singleton/saved_variables
...
/**
 * Caches the /singleton/saved_variables type matching a given /datum type path.
 * Key is /datum typepath, value is the type of the matching /singleton/saved_variables.
*/
...
/proc/get_saved_decl(var/path)
	var/singleton/saved_variables/SV
...
/proc/get_saved_variables_for(var/path)
	...
	var/singleton/saved_variables/SV = get_saved_decl(path)
 ...
 * Objects of this type expect the path of the entity they contain info on to have their full path in the datum subtype's path. (Ex: /mob/living -> /singleton/saved_variables/mob/living )
 */


/singleton/saved_variables
...
/singleton/saved_variables/Initialize()
	. = ..()
...
*/
/singleton/saved_variables/proc/make_saved_variables(var/list/saved = list())
	cached = saved
	if(ispath(base_type))
...
/singleton/saved_variables/proc/get_saved_variables()
	if(!cached)
...
/singleton/saved_variables/proc/test_variables(var/datum/instance)
...
/singleton/saved_variables/datum
	type_path = /datum
	base_type = null //Datum is the base type of everything, so nothing here.

//Atoms
/singleton/saved_variables/atom
	type_path = /atom

/singleton/saved_variables/turf
	type_path = /turf
	base_type = /atom

/singleton/saved_variables/area
	type_path = /area
	base_type = /atom

//Movables
/singleton/saved_variables/atom/movable
	type_path = /atom/movable

/singleton/saved_variables/obj
	type_path = /obj
	base_type = /atom/movable

/singleton/saved_variables/mob
	type_path = /mob
	base_type = /atom/movable

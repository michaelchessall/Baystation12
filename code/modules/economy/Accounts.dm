/proc/GetPersonalMoneyAccount(var/realname)
	for(var/datum/money_account/account in all_money_accounts)
		if(account.account_type == ACCOUNT_TYPE_PERSONAL && account.owner_name == realname)
			return account
/datum/money_account
	var/account_name = ""
	var/owner_name = ""
...
/proc/create_faction_account(account_name = "Default account name", owner_name, starting_funds = 0, account_type = ACCOUNT_TYPE_DEPARTMENT, obj/machinery/computer/account_database/source_db)
